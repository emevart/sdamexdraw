...
- Перетаскивание первой или последней точки открытой линии в редакторе к другому концу (ближе 20 экранных px) замыкает её в многоугольник, как при рисовании. Кольцо-индикатор замыкания показывается и при рисовании, и при перетаскивании конца, пока отпускание замкнёт линию, и больше не висит на многоугольнике в редакторе. Стрелки не замыкаются [#16](https://github.com/emevart/sdamexdraw/pull/16).
...
- «Привязка к середине» стрелок, её индикаторы и округление точки крепления стрелки к сетке зависят от «Привязки к сетке», а не от показа сетки: при показанной сетке без привязки к ней стрелки крепятся как без сетки. Новый предикат `isGridSnappingEnabled(appState)` в `@excalidraw/element`, `InteractiveCanvasAppState` получает `gridSnapEnabled` [#16](https://github.com/emevart/sdamexdraw/pull/16).

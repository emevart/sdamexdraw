...
class App extends React.Component<AppProps, AppState> {
  ...
  private animateStraighten(element: NonDeleted<ExcalidrawFreeDrawElement>) {
  ...
  private _handleWireframeVertexDrag = (pointerCoords: {
    ...
    if (
      ...
      // #11470 сузил NonDeleted до `isDeleted: false`: movePoints больше не
      // принимает удалённый элемент. Тянуть вершину удалённого нечего -- жест
      // сбрасывается вместе с остальными негодными случаями.
            ...
            straightenTimerId = window.setTimeout(() => {
              ...
              if (
                ...
                this.animateStraighten(
                  el as NonDeleted<ExcalidrawFreeDrawElement>,
                );
              }
            }, STRAIGHTEN_HOLD_TIME);
          }
  ...
  private handleWheel = withBatchedUpdates(
    (
      ...
      //
      // while rubberband-overscrolled past the scroll constraints, suppress
      // zooming until the viewport has snapped back inside the box. Panning
      // (shift+wheel above) stays allowed -- it is how one gets back.
      if (isViewportOverscrolled(this.state)) {
        return;
      }
      ...
      // a locked viewport must not be zoomed out past the level it was
      // locked at, otherwise the constrained box stops filling the screen

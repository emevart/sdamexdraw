...
- Пунктир и точки у пера: стиль линии в панели свойств доступен и для свободного рисования. Сплошной штрих остаётся залитым контуром, который следует нажиму; пунктирный и точечный рисуются по средней линии ровной шириной — шириной чернил при среднем нажиме, на холсте и в экспорте SVG (`isDashedFreedraw`, `getFreeDrawCenterlineSvgPath`, `getFreedrawDashWidth`, `getFreedrawDashArray` в `@excalidraw/element`). Касание и щелчок рисуют точку, замкнутый штрих (круг, «о») остаётся контуром. Стиль хранится в наборе инструмента, в том числе при открытии редактора с восстановленным пером: пунктир пера не переходит на фигуры и маркер. У пера в панели есть стиль линии, но нет «Небрежности» (`hasSloppiness`): на чернила она не влияет и меняла бы следующие фигуры. `ToolStrokeSettings` получает необязательное `strokeStyle`; `getToolSettings` и `onToolSettingsChange` отдают его, `setToolSettings` без поля стиль не меняет, неизвестное значение отбрасывается [#18](https://github.com/emevart/sdamexdraw/pull/18).
- Восемь ручек изменения размера на всех устройствах, включая планшет: средние ручки рисуются и на iPad, планшет по-прежнему тянет и за полосу у стороны. Средние ручки скрываются, если сторона фигуры на экране короче 44 px (`MIDDLE_HANDLES_MIN_SIDE_PX`, прежде 40 px). Где зоны ручек перекрываются (палец: зона 28 px, поворот в 16 px над средней верхней ручкой), выбирается ручка с ближайшим центром: раньше касание чуть выше средней ручки поворачивало фигуру. У мыши и пера зоны не перекрываются, поведение прежнее [#18](https://github.com/emevart/sdamexdraw/pull/18).
...
- Отпускание пера или мыши во время анимации выпрямления (250 мс после удержания) фиксирует целевую форму — прямую или сглаженную кривую, как после конца анимации, без точки отпускания. Раньше в сцену уходил полувыпрямленный штрих с крючком на конце [#16](https://github.com/emevart/sdamexdraw/pull/16).
- Перетаскивание первой или последней точки открытой линии в редакторе к другому концу (ближе 20 экранных px) замыкает её в многоугольник, как при рисовании. Прилипают только линии от четырёх точек, и при рисовании тоже: угол V из трёх точек не схлопывается в отрезок. Кольцо-индикатор замыкания показывается и при рисовании, и при перетаскивании конца, пока отпускание замкнёт линию; на замкнутой линии в редакторе кольцо стоит на стыке, как раньше. Стрелки не замыкаются [#16](https://github.com/emevart/sdamexdraw/pull/16).
- Подпись «Шаг сетки» и заголовок «Холст» в панели «Свойства» переводятся (ключи `stats.gridStep`, `stats.canvas`) [#16](https://github.com/emevart/sdamexdraw/pull/16).
- «Привязка к середине» стрелок, её индикаторы и округление точки крепления стрелки к сетке зависят от «Привязки к сетке», а не от показа сетки: при показанной сетке без привязки к ней стрелки крепятся как без сетки. Так же Ctrl при перетаскивании включает привязку к объектам, а подсказка «Удерживайте Ctrl, чтобы отключить привязку» не показывается, пока сетка только показана. Новый предикат `isGridSnappingEnabled(appState)` в `@excalidraw/element`, `InteractiveCanvasAppState` получает `gridSnapEnabled` [#16](https://github.com/emevart/sdamexdraw/pull/16).
...
- Когда хост засеял настройки инструментов, после любой смены инструмента в обход тулбара (Esc, снятие замка, возврат из ластика или руки) текущие цвет, толщина и прозрачность берутся из набора нового инструмента: правка выделенной фигуры больше не переносит в набор фигур толщину и прозрачность маркера или карандаша [#15](https://github.com/emevart/sdamexdraw/pull/15).
- Очистка холста сохраняет цвет, толщину и прозрачность текущего инструмента: набор настроек не сбрасывается к умолчаниям, и `onToolSettingsChange` не зовётся [#15](https://github.com/emevart/sdamexdraw/pull/15).
...
- Статический канвас больше не перерисовывается, если пакет изменений (например, чужие правки в совместной сессии) не затронул то, что он рисует. У статики свой нонс: он меняется при смене состава или порядка видимых элементов, при смене объекта, `version` или `versionNonce` у видимого элемента, его фрейма или подписи, а также при явном `scene.triggerUpdate()`. Мутация элемента хостом без смены объекта и версии статику не перерисует: нужна новая версия или `scene.triggerUpdate()`. Отложенная отрисовка статики троттлится по канвасу, размонтирование одного редактора не отменяет отрисовку другого [#13](https://github.com/emevart/sdamexdraw/pull/13).
- Перерастрирование элементов после зума идёт под бюджетом 8 мс на кадр и доделывается в следующих кадрах; до этого растр от прежнего масштаба рисуется со сглаживанием, поэтому несколько кадров он может выглядеть масштабированным [#13](https://github.com/emevart/sdamexdraw/pull/13).
- Стеки undo и redo ограничены `History.maxEntries` (300), старые записи отбрасываются; записи о выделении тоже занимают место в стеке [#13](https://github.com/emevart/sdamexdraw/pull/13).
